...
public class GroundPlaneUIRes : MonoBehaviour
{
    ...
    ProductPlacementRes m_ProductPlacement;
    TouchHandlerRes m_TouchHandler;
    ...
    void Update()
    {
        if (m_ProductPlacement.IsPlaced || PlaneManagerRes.AstronautIsPlaced)
        {
    ...
    void LateUpdate()
    {
        if (PlaneManagerRes.GroundPlaneHitReceived)
        {
            ...
            if (PlaneManagerRes.planeMode == PlaneManagerRes.PlaneMode.GROUND)
            {
                m_Instructions.text = "Tap to place Astronaut";
            }
            else if (PlaneManagerRes.planeMode == PlaneManagerRes.PlaneMode.PLACEMENT)
            {
        ...
        else
        {
            ...
            m_ScreenReticle.alpha =
                (PlaneManagerRes.planeMode == PlaneManagerRes.PlaneMode.GROUND ||
                PlaneManagerRes.planeMode == PlaneManagerRes.PlaneMode.PLACEMENT) ? 1 : 0;

            m_Instructions.transform.parent.gameObject.SetActive(true);
            m_Instructions.enabled = true;

            if (PlaneManagerRes.planeMode == PlaneManagerRes.PlaneMode.GROUND ||
                PlaneManagerRes.planeMode == PlaneManagerRes.PlaneMode.PLACEMENT)
            {
                m_Instructions.text = "Point device towards ground";
            }
            else if (PlaneManagerRes.planeMode == PlaneManagerRes.PlaneMode.MIDAIR)
            {
    ...
    {
        switch (PlaneManagerRes.planeMode)
        {
            case PlaneManagerRes.PlaneMode.GROUND:
                m_Title.text = TITLE_GROUNDPLANE;
                break;
            case PlaneManagerRes.PlaneMode.MIDAIR:
                m_Title.text = TITLE_MIDAIR;
                break;
            case PlaneManagerRes.PlaneMode.PLACEMENT:
                m_Title.text = TITLE_PLACEMENT;
                break;
        }
    }
